...
function HillCipherDecryptor() {
   ...
   function decryptHillCipher(encryptedText, keyMatrix) {
      let decryptedText = '';
      ...
      let adj = [
         [
            (keyMatrix[1][1] * keyMatrix[2][2] - keyMatrix[2][1] * keyMatrix[1][2]) % 26,
         ...
         [
            ...
            (keyMatrix[1][0] * keyMatrix[0][2] - keyMatrix[0][0] * keyMatrix[1][2]) % 26,
         ],
         [
            (keyMatrix[1][0] * keyMatrix[2][1] - keyMatrix[2][0] * keyMatrix[1][1]) % 26,
            (keyMatrix[2][0] * keyMatrix[0][1] - keyMatrix[0][0] * keyMatrix[2][1]) % 26,
            (keyMatrix[0][0] * keyMatrix[1][1] - keyMatrix[1][0] * keyMatrix[0][1]) % 26,
         ],
      ];

      let inverseMatrix = adj.map((row) => row.map((el) => mod(el * detInv, 26)));
      ...
      for (let i = 0; i < encryptedText.length; i += 3) {
         ...
         let resultVector = [0, 0, 0];
         for (let j = 0; j < 3; j++) {
            for (let k = 0; k < 3; k++) {
               resultVector[j] = mod(resultVector[j] + inverseMatrix[j][k] * vector[k], 26);
            }
   ...
   return (
      <div>
         ...
         <input type="text" value={encryptedText} onChange={(e) => setEncryptedText(e.target.value)} placeholder="Enter encrypted text" />
